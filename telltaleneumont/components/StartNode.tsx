...
export default function StartNode() {
  ...
  const PAGES = {
    good: 2,     // correct JS console print
    neutral: 6,  // exact printed output only
    bad: 10,      // anything else
  };
  // ----------------------------------
          ...
          <div className="mt-4 min-h-[2.5rem]" aria-live="polite" aria-atomic="true">
            {result === "good" && (
              <div className="rounded-lg bg-green-50 border border-green-200 px-3 py-2 text-sm text-green-800">
                Well done — go to page <b>{PAGES.good}</b>.
              </div>
            )}
            {result === "neutral" && (
              <div className="rounded-lg bg-yellow-50 border border-yellow-200 px-3 py-2 text-sm text-yellow-800">
                Not bad — go to page <b>{PAGES.neutral}</b>.
              </div>
            )}
            {result === "bad" && (
              <div className="rounded-lg bg-red-50 border border-red-200 px-3 py-2 text-sm text-red-800">
                Not a great start — go to page <b>{PAGES.bad}</b>.
              </div>
            )}
          </div>
